...
RUN wget -q "https://github.com/moby/buildkit/releases/download/v0.10.6/buildkit-v0.10.6.linux-${BUILDARCH}.tar.gz" && mkdir buildkit && cat buildkit-v0.10.6.linux-${BUILDARCH}.tar.gz | tar -C buildkit -zxvf - && mv buildkit/bin/buildctl /usr/bin/buildctl && rm -rf buildkit
