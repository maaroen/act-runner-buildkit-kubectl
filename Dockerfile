...
ARG TARGETARCH
RUN wget -q "https://github.com/moby/buildkit/releases/download/v0.12.3/buildkit-v0.12.3.linux-${TARGETARCH}.tar.gz" && mkdir buildkit && cat buildkit-v0.12.3.linux-${TARGETARCH}.tar.gz | tar -C buildkit -zxvf - && mv buildkit/bin/buildctl /usr/bin/buildctl && rm -rf buildkit
